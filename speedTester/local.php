...
if (isset($_REQUEST['submit'])) {
  $submit = $_REQUEST['submit'];
    ...
    $executionTimeEnd = microtime(true); // timer-end
		$message = '<hr><b>Total Execution Time:</b> ' . $stopwatch->getExecutionTime($executionTimeStart, $executionTimeEnd) . '';
	} else {
    ...
    foreach($folder as $item){    				
        ...
        if (is_dir($itemPath)) {
			...
			if ($path_parts['extension'] === 'php' and file_exists($itemPath)) {
				...
				echo '<li>' . $appRootDir . '/<strong><a href="'. $rootTestFolderName . '/' . str_replace($originAppRootDir, '', $appRootDir) . '/' . $item . '">' . $item . '</a></strong> [' . $stopwatch->getExecutionTime($executionTimeStart, $executionTimeEnd) . ']';
                if (!empty($log))
                {
                    foreach ($bugTypes as $bug)
                    {
